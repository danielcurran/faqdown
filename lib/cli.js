...
function parseArgs(argv, opts = {}) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    for (const name of Object.keys(knownFlags)) {
      ...
      if (arg === '--' + name) {
        // Boolean flag: no value expected, set to true
        if (knownFlags[name].value === '') {
          flags[name] = true;
          matched = true;
          break;
        }
        // Value flag: consume next arg unless it looks like another flag
        const next = argv[i + 1];
 ...
 * Resolves symlinks on both sides to prevent symlink-bypass attacks.
 */
function validateOutputPath(outputPath, allowedBases) {
  const resolved = fs.existsSync(outputPath)
    ? fs.realpathSync(outputPath)
    : fs.realpathSync(path.dirname(outputPath)) + path.sep + path.basename(outputPath);
  for (const base of allowedBases) {
    const baseResolved = fs.existsSync(base)
